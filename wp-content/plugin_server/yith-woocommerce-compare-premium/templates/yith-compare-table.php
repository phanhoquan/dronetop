...
<div id="yith-woocompare" class="woocommerce">
	...
	if ( empty( $products ) ):
						...
						<td class="<?php echo esc_attr( $product_class ); ?>"><?php
							switch ( $field ) {
								...
								case 'product_info':
									...
									if ( ! $fixed )
										echo '<div class="remove"><a target="_blank" href="' . esc_url( $yith_woocompare->obj->remove_product_url( $product_id ) ) . '" data-iframe="' . esc_attr( $iframe ) . '" data-product_id="' . esc_attr( $product_id ) . '"><span class="remove">' . wp_kses_post( apply_filters( 'yith_woocompare_remove_icon', 'x' ) ) . '</span>' . wp_kses_post( apply_filters( 'yith_woocompare_remove_label', esc_html__( 'Remove', 'yith-woocommerce-compare' ) ) ) . '</a></div>';

									if ( $show_image || $show_title ) {
										echo '<a target="_blank" href="' . esc_attr( $product->get_permalink() ) . '">';
										if ( $show_image )
	    ...
	    <tr class="btn-link-add-to-cart">
			<th>View detail product</th>
			<?php
			$index = 0;
			foreach ( $products as $product_id => $product ) :
				...
				?>
				<td class="<?php echo esc_attr( $product_class ); ?>">
					<a rel="nofollow" target="_blank" href="<?php echo $link_product_cart_detail;?>" class="button product_type_simple add_to_cart_button cart-link"><?php echo $text_Button_text; ?></a>
				</td>
				<?php
				++$index;
			endforeach;
