<?php
add_action('widgets_init', 'logistics_tweets_load_widgets');

function logistics_tweets_load_widgets()
{
    register_widget('logistics_Tweets_Widget');
}

class logistics_Tweets_Widget extends WP_Widget {

    function __construct()
    {
        parent::__construct(
            'zo-tweets-widget',
            esc_html__('Zo Twitter','am-logistics'),
            array('description' => esc_html__('Twitter New Status Widget.', 'am-logistics'),)
        );
    }
    ...
    function widget($args, $instance)
    {
        ...
        if(!empty($instance['twitter_extra_class'])){
            $twitter_extra_class = $instance['twitter_extra_class'];
        }else{
            $twitter_extra_class = "";
        }
        ...
        if($twitter_id && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) {
            ...
            if(false === ($twitterData = get_transient($transName))) {
                ...
                if(!$token) {
                    ...
                    $toSend = call_user_func( $function , $credentials );
            ...
            if($twitter && is_array($twitter)) {
                ?>
                <div class="twitter-box <?php echo esc_attr($twitter_extra_class);?>">
                                        ...
                                        <li class="jtwt_tweet">
                                            ...
                                            <div class="tweet_right">
                                                ...
                                                <?php if($show_date == 'yes'): ?>
                                                    <a href="http://twitter.com/<?php echo esc_attr($tweet['user']['screen_name']); ?>/statuses/<?php echo esc_attr($tweet['id_str']); ?>" class="jtwt_date"><?php esc_html_e('about','am-logistics');?> <?php echo esc_attr($timeAgo); ?></a>
                                                <?php endif; ?>
                                                <p class="jtwt_tweet_text">
                                                    <?php
                                                    $latestTweet = $tweet['text'];;
                                                    $latestTweet = preg_replace('/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/', '<a href="$0" target="_blank">$0</a>', $latestTweet);

                                                    //Convert hashtags to twitter searches in <a> links
                                                    $latestTweet = preg_replace("/#([A-Za-z0-9\/\.]*)/", "<a target=\"_new\" href=\"http://twitter.com/hashtag/$1\?src=hash\">#$1</a>", $latestTweet);

                                                    //Convert attags to twitter profiles in &lt;a&gt; links
                                                    $latestTweet = preg_replace("/@([A-Za-z0-9\/\.]*)/", "<a href=\"http://www.twitter.com/$1\">@$1</a>", $latestTweet);
    ...
    function update($new_instance, $old_instance)
    {
        ...
        $instance['twitter_extra_class'] = $new_instance['twitter_extra_class'];
    ...
    function form($instance)
    {
        ...
        <p>
            ...
            <select id="<?php echo esc_attr($this->get_field_id('show_avatar')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_avatar')); ?>">
                <option value="no" <?php if($instance['show_avatar'] == 'no'){ echo "selected='selected'"; } ?>><?php echo esc_html__('No','am-logistics'); ?></option>
                <option value="yes" <?php if($instance['show_avatar'] == 'yes'){ echo "selected='selected'"; } ?>><?php echo esc_html__('Yes','am-logistics'); ?></option>
            </select>
        </p>
        <p>
            ...
            <select id="<?php echo esc_attr($this->get_field_id('show_date')); ?>" class="widefat" name="<?php echo esc_attr($this->get_field_name('show_date')); ?>">
                <option value="no" <?php if($instance['show_date'] == 'no'){ echo "selected='selected'"; } ?>><?php echo esc_html__('No','am-logistics'); ?></option>
                <option value="yes" <?php if($instance['show_date'] == 'yes'){ echo "selected='selected'"; } ?>><?php echo esc_html__('Yes','am-logistics'); ?></option>
            </select>
        </p>
        <p>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('twitter_extra_class')); ?>">Extra Class:</label>
            <input class="widefat" type="text" id="<?php echo esc_attr($this->get_field_id('twitter_extra_class')); ?>" name="<?php echo esc_attr($this->get_field_name('twitter_extra_class')); ?>" value="<?php if(!empty($instance['twitter_extra_class'])){ echo esc_attr($instance['twitter_extra_class']); }?>" />
        </p>
    <?php
    }
}
