...
<div class="zo-grid-wrapper <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
    ...
    <?php if ( isset($atts['filter']) && $atts['filter'] == 1 ): ?>
        <div class="zo-grid-filter">
            <ul class="zo-filter-category list-unstyled list-inline">
                <li><a class="active" href="#" data-group="all"><?php esc_html_e("All", 'am-logistics');?></a></li>
                <?php
                $posts = $atts['posts'];
                $query = $posts->query;
                $taxs  = array();
    ...
    <div class="row zo-grid <?php echo esc_attr($atts['grid_class']);?>">
        ...
        while($posts->have_posts()){
            ...
            ?>
            <div class="zo-grid-item <?php echo esc_attr($atts['item_class']);?>" data-groups='[<?php echo implode(',', $groups);?>]' style="<?php echo esc_attr($item_style);?>">
                <div class="zo-grid-inner">
                   <div class="zo-grid-media">
                    ...
                    <div class="zo-grid-readmore">
                        <a href="<?php echo the_permalink();?>"><?php echo esc_html__('Read more','am-logistics');?></a>
                    </div>
                </div>
            </div>
            <?php
